...
gen n_cat=0
replace n_cat=1 if hund==1 
replace n_cat=2 if five==1
replace n_cat=3 if n_limit==1


estimates clear

reghdfe ptb ib0.n_cat i.($controls) c.dmage  [pw=w] , vce(cluster fips)  absorb(fips#year year#month)

test 1.n_cat = 2.n_cat = 3.n_cat = 0 
****Tests****

estimates clear
reghdfe lbw ib0.n_cat i.($controls) c.dmage i.dgestat [pw=w] , vce(cluster fips)  absorb(fips#year year#month)
